...
namespace iop
{
    ...
    public:
        ...
        Component(unsigned int subsystem, unsigned short node, unsigned short component, rclcpp::Node& rosnode);
        virtual ~Component();
    ...
    protected:
        ...
        rclcpp::Node* p_rosnode;
        virtual void processInternalEvent(JTS::InternalEvent* ie);

        JausAddress p_own_address;
        std::shared_ptr<iop::PluginInterface> p_discovery_client;
        std::map<std::string, std::string > p_service_package_list;
        pluginlib::ClassLoader<iop::PluginInterface>* p_class_loader;
        std::map<std::string, std::shared_ptr<iop::PluginInterface> > p_plugins_map;
        std::map<std::string, std::shared_ptr<iop::PluginInterface> > p_plugins_empty;
	...
	rclcpp::Publisher<diagnostic_msgs::msg::DiagnosticArray>::SharedPtr p_publisher_diagnostics;
        ...
        void load_plugins();
        std::shared_ptr<iop::PluginInterface> p_init_plugin(std::string name, pluginlib::ClassLoader<iop::PluginInterface>& class_loader);
