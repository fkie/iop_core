...
void Slave::pInitRos()
{
    ...
    cfg.param<std::string>("control_addr", control_addr, control_addr, true,
    ...
    if (!control_addr.empty()) {
        ...
        cfg.param<bool>("force_monitor_on_start", p_force_monitor_on_start, p_force_monitor_on_start, true,
            rcl_interfaces::msg::ParameterType::PARAMETER_BOOL,
            "Start monitoring the component declared in 'control_addr' immediately. No effect if 'control_addr' is not set!",
            "Default: false");
    }
    cfg.param<int64_t>("authority", p_default_authority, p_default_authority, true,
        rcl_interfaces::msg::ParameterType::PARAMETER_INTEGER,
        "The authority for access control.",
        "Default: 205");
    std::map<int64_t, std::string> access_control_map;
    ...
    cfg.param_named<int64_t>("access_control", p_default_access_control, p_default_access_control, access_control_map, true,
        rcl_interfaces::msg::ParameterType::PARAMETER_INTEGER,
    ...
    cfg.param<bool>("use_queries", p_use_queries, p_use_queries, true,
        rcl_interfaces::msg::ParameterType::PARAMETER_BOOL,
        "Enables query behaviour. Reports are send only as reply to own query.",
        "Default: false");
    cfg.param<bool>("only_monitor", p_only_monitor, p_only_monitor, true,
        rcl_interfaces::msg::ParameterType::PARAMETER_BOOL,
        "Disables the ability to send RequestControl and uses client only for monitoring if true",
        "Default: false");
    cfg.param<int64_t>("subsystem_restricted", p_subsystem_restricted, p_subsystem_restricted, true,
        rcl_interfaces::msg::ParameterType::PARAMETER_INTEGER,
        "This client can control only the specified subsystem (robot). Any by default.",
        "Default: 65535");
    cfg.param<int64_t>("controlled_component", p_controlled_component_nr, p_controlled_component_nr, true,
        rcl_interfaces::msg::ParameterType::PARAMETER_INTEGER,
    ...
    for (unsigned int i = 0; i < control->cmds.size(); i++) {
        ...
        if (p_current_control_addr.get() != 0) {
            if (!control_addr.match(p_current_control_addr) && cmd.access_control != Component::ACCESS_CONTROL_RELEASE) {
                RCLCPP_WARN(logger, "already controlling %s, cannot switch to %s", p_current_control_addr.str().c_str(), control_addr.str().c_str());
                skipped = true;
                continue;
            } else if (p_current_control_state == cmd.access_control) {
                skipped = true;
                RCLCPP_WARN(logger, "state for %s already applied", p_current_control_addr.str().c_str());
                continue;
            }
        }
            ...
            if (cmp_addr.match(addr)) {
                ...
                if (cmp.set_access_control(it->second.first) or cmp.set_authority(it->second.second)) {
                    ...
                    case Component::ACCESS_CONTROL_RELEASE:
                        RCLCPP_DEBUG(logger, "apply command ACCESS_CONTROL_RELEASE to %s", cmp_addr.str().c_str());
                    ...
                    case Component::ACCESS_CONTROL_MONITOR:
                        RCLCPP_DEBUG(logger, "apply command ACCESS_CONTROL_MONITOR to %s", cmp_addr.str().c_str());
                        cmp.set_state(Component::ACCESS_STATE_MONITORING);
                        break;
                    case Component::ACCESS_CONTROL_REQUEST:
                        RCLCPP_DEBUG(logger, "apply command ACCESS_CONTROL_REQUEST to %s", cmp_addr.str().c_str());
                        // send request access
                        if (pGetAccesscontrolClient() != 0) {
                            cmp.set_authority(it->second.second);
                            request_access(cmp_addr, it->second.second);
                        } else {
                            RCLCPP_WARN(logger, "no acces control available -> set state to ACCESS_CONTROL_MONITOR to %s", cmp_addr.str().c_str());
                            cmp.set_state(Component::ACCESS_STATE_MONITORING);
                        }
                        break;
                    }
